function [nu] = CalculateViscosity(pressure,Temperature,rho_x)
% CalculateViscosity calculates the kinematic viscosity for each pipe.

% For the calculation of the viscosity two different methods were combined.

% First one: A formular for the calculation of the compressibility of gases. 
% This method only works well for "little" pressures up to 50 - 100 bar.

% Second one: Inperpolation of the values based on a table with values out of the
% VDI-Wärmeatlas. This method only works well for "high" pressures
% beginning from 50 - 100 bar.

% As method one is working for small pressures and method two is working
% for high pressures we decided to use both. The function decides to one of
% them depending on the input-pressure.
...
% OUTPUT: 
%   - kinematic viscosity [m^2/s]*10^-6


p = pressure/(10^5);        % translation from pascal to bar 
T = Temperature-273.15;     % translation from Kelvin to Celsius 

% Initial vectors for formular-calculations
nu = zeros(size(p));
p_c = zeros(size(p));
p_c(1:size(p)) = 39.5;      % critical pressure [bar] (always constant)
T_c = zeros(size(p)); 
T_c(1:size(p)) = 133;       % critical temperature [K] (always constant)
M = zeros(size(p));
M(1:size(p)) = 28.949;      % molar mass [g/mol] (always constant)
...
for i = 1:size(p)
    if p(i)>= 1 && p(i)<50
        ...
        % viscosity at 1 bar in 10^-7 Pas at a certain temperature
        % interpolation of the temperature at 1 bar from the VDI-Wärmeatlas
        ...
        vis_1bar(i) = (F(T(i))*10^(-6))/10^(-7);
        
        % Calculation of p_r 
        p_r(i) = rho_x(i)/313; % p_r = rho_x divided by the density at the critical point [Kg/m^3]

        % Calcultation of A 
        A(i) = 1.023 + 0.23364*p_r(i) + 0.58533*p_r(i)^2 - 0.40758*p_r(i)^3 + 0.093324*p_r(i)^4;

        % Main Formular
        dyn_vis(i) = vis_1bar(i) + (A(i)^4 - 1)*((p_c(i)^(2/3)*M(i)^(0.5))/T_c(i)^(1/6));
        dyn_vis(i) = dyn_vis(i)/10;
        kin_vis(i) = dyn_vis(i)/rho_x(i);
        nu(i) = kin_vis(i)*10^(-6);

    elseif p(i) >= 50 && p(i) <= 1000
        ...
        % Z:kinematic viscosity in 10^-7 m/s(values from VDI-Wärmeatlas (messured values))
        ...
        nu(i) = F({T(i),p(i)})*10^(-7); % gives back the interpolated nu-value
        
    else
        nu(i) = 1*10^(-7)% without a nu-value it's difficult to find a solution -> default value
